...
//!
//! When `chars` is unset and the terminal speaks kitty graphics, sixel,
//! or iTerm2 inline images, an image logo prints as a real image.
//! Otherwise — unsupported terminal, piped output, or `chars=ascii` —
//! it falls back to the half-block / ascii conversion in
//! [`super::image`].
//!
//! Only the one-shot static path uses this: animated (3D) output keeps
//! the point cloud (a native image can't rotate), and live mode keeps
//! the redraw-safe half-blocks.
